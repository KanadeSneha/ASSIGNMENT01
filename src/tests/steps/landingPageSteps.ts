...
Then('I should be able to see dashborad page', async function () {
...
Given('I click on ToDo content textbox', async function () {
...
Given('I enter text content {string}', async function (text: string) {
  await this.page.getByRole('textbox', { name: 'Todo content' }).click();
  await this.page.getByRole('textbox', { name: 'Todo content' }).fill(text);
});

When('I click on Create Todo button', async function () {
  await this.page.getByRole('button', { name: 'Create ToDo' }).click();

});

Then('I should be able to see {string} in ToDo item to list', async function (text: string) {
...
When('I click on Due date field', async function () {
  await this.page.locator('#mat-input-1').click();
});


When('clear the Due date', async function () {
  await this.page.locator('#mat-input-1').fill('');
});

When('add current date', async function () {
  await this.page.fill('#mat-input-1', getCurrentDateISO());
});

When('I select a due date {int} days in the past', async function (days: number) {
...
Then('Create ToDo button should be disabled', async function () {
